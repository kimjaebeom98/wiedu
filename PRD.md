...
> 이 섹션은 PRD와 실제 구현 간의 차이를 명확히 합니다.

### 구현 완료 (MVP)

| 기능 | 상태 | 비고 |
|------|------|------|
| 카카오 로그인 | ✅ 완료 | OAuth 3-legged flow |
| 이메일 로그인/가입 | ✅ 완료 | 인증코드 방식 |
| 스터디 CRUD | ✅ 완료 | 6단계 생성 플로우 |
| 스터디 검색 | ✅ 완료 | 키워드 + 카테고리 필터 |
| 근처 스터디 | ✅ 완료 | 프로필 좌표 기반 (fallback 포함) |
| 인기 스터디 | ✅ 완료 | 충원율 기준 (fallback 포함) |
| 가입 신청/승인 | ✅ 완료 | 리더 승인제 |
| 스터디 게시판 | ✅ 완료 | 공지/잡담/Q&A + 댓글 |
| 사진첩 | ✅ 완료 | 멤버 전용 |
| 리뷰 시스템 | ✅ 완료 | 리더 리뷰 + 멤버 상호 리뷰 |
| 온도 시스템 | ✅ 완료 | 글로벌 + 카테고리별 |
| 프로필 관리 | ✅ 완료 | 닉네임, 소개, 지역, 이미지 |
| 온보딩 | ✅ 완료 | 6단계 플로우 |
...
| 기능 | 상태 | 비고 |
|------|------|------|
| 개인 스터디 기능 | ❌ 기획 완료 | 📄 [PRD-개인스터디.md](./PRD-개인스터디.md) 참조 |
| 네이버/구글/애플 로그인 | ❌ 스텁 | "준비 중" 표시 |
...
> **현재 구현**: 홈 화면 중심의 단일 스택 네비게이션 (바텀 탭 없음)

```
┌─────────────────────────────────────────────────────────────┐
│                        App Screen                            │
├─────────────────────────────────────────────────────────────┤
│  홈 화면 → 각 기능은 헤더 아이콘/버튼으로 접근                  │
└─────────────────────────────────────────────────────────────┘
```
...
5. 관심 분야 선택
6. 경험 수준 선택 (초급/중급/고급)
7. 선호 스터디 방식 (온라인/오프라인)
8. 지역 설정 (카카오맵 검색)
9. 가입 완료
```

**비밀번호 찾기 플로우:** *(Phase 2 예정)*
```
❌ 현재 미구현 - 이메일 재설정 기능 없음
```
...
**Acceptance Criteria:**
- [x] 2가지 로그인 방식 동작 (카카오, 이메일)
- [x] 중복 이메일/닉네임 체크
- [x] 약관 동의 처리
- [x] 자동 로그인 (토큰 저장)
- [x] 이메일 인증코드 발송 및 검증
- [ ] 비밀번호 재설정 링크 발송 및 처리 *(Phase 2)*
- [x] 인증코드 만료 처리 (5분)
...
| 필터 | 옵션 |
|------|------|
| 분야 (대분류) | 어학, 취업/이직, IT/개발, 자격증, 공무원/고시, 재테크, 디자인, 비즈니스, 학습/교육, 라이프스타일, 콘텐츠/창작 |
...
**스터디 카테고리 체계 (11개 대분류, 89개 중분류):**

| 대분류 | 중분류 (세부 주제) |
|--------|-------------------|
| 어학 | 토익, 토플, IELTS, OPIC, 영어회화, 비즈니스영어, 일본어(JLPT), 중국어(HSK), 스페인어, 프랑스어, 독일어, 기타언어 |
| 취업/이직 | 자소서 첨삭, 면접 스터디, 포트폴리오, 이력서 작성, 직무분석, 인적성검사, 기업분석, 커리어전환 |
| IT/개발 | 코딩테스트, 프론트엔드, 백엔드, 풀스택, 모바일앱, AI/ML, 데이터분석, 클라우드/DevOps, 보안, 게임개발, 블록체인, CS기초, 사이드프로젝트 |
| 자격증 | 정보처리기사, AWS자격증, 컴활, SQLD, 빅데이터분석기사, 네트워크관리사, CPA, 세무사, 공인중개사, 주택관리사 |
| 공무원/고시 | 9급공무원, 7급공무원, 경찰공무원, 소방공무원, 군무원, 법원/검찰, 교육행정직, 세무직, 법무사, 행정사 |
| 재테크 | 주식투자, ETF, 부동산투자, 경매/공매, 가상자산, 경제공부, 재무설계, 세테크, 해외투자 |
| 디자인 | UI/UX, 그래픽디자인, 영상편집, 3D/모션, 일러스트, 포토샵/라이트룸, Figma/Sketch, 브랜딩 |
| 비즈니스 | 창업, 마케팅, PM/기획, 데이터분석, 브랜딩, 사업계획서, 투자유치, 커머스 |
| 학습/교육 | 수능/내신, 편입, 대학원, 논문작성, 교원임용, 독서토론, 글쓰기, 자기계발 |
| 라이프스타일 | 운동/헬스, 요가/필라테스, 러닝/마라톤, 등산, 악기/음악, 요리/베이킹, 수공예, 사진촬영, 여행, 반려동물 |
| 콘텐츠/창작 | 유튜브, 블로그/브런치, 인스타그램, 팟캐스트, 웹툰/웹소설, 전자책, SNS마케팅, 크리에이터 |
...
#### 4.3.9 외부 채팅 연동 *(Phase 2로 연기)*

> **현재 상태**: 미구현. 스터디 내 소통은 **게시판(공지/잡담/Q&A)**으로 대체.
...
**Acceptance Criteria:**
- [ ] 카카오톡 오픈채팅 링크 저장/수정 *(Phase 2)*
- [ ] 외부 링크 클릭 시 앱 이동 *(Phase 2)*
- [x] 게시판을 통한 비동기 소통 (현재 구현됨)
...
#### 4.3.10 친구 신청 *(Phase 2 - 미구현)*

> **현재 상태**: 미구현. 스터디 멤버 간 소통은 게시판으로 대체.
...
| 상태 | 설명 |
|------|------|
| 미참여 | 해당 카테고리 스터디 참여 이력 없음 |
| 활성화 | 첫 참여 시 36.5°C로 시작, 활동에 따라 온도 상승 |

**온도 등급:** *(UI 표시용, Phase 2에서 적용)*
| 등급 | 온도 범위 | 배지 |
|------|----------|------|
| 🌱 새싹 | 36.5~40°C | 입문자 |
| 🌿 성장 | 40~50°C | 학습자 |
...
| 활동 | 온도 변화 |
|------|----------|
| 해당 분야 스터디 완주 | +0.1°C (카테고리 온도) |
| 멤버 리뷰 5점 | +0.5°C (글로벌 온도) |
| 멤버 리뷰 4점 | +0.2°C |
| 멤버 리뷰 3점 | 0°C |
| 멤버 리뷰 2점 | -0.2°C |
| 멤버 리뷰 1점 | -0.5°C |
...
**Acceptance Criteria:**
- [ ] FCM 연동 (iOS/Android) *(Phase 2)*
- [ ] 인앱 알림 목록 표시 *(Phase 2)*
- [ ] 알림 읽음/삭제 처리 *(Phase 2)*
- [x] 알림 설정 저장 (DB에 저장됨, 실제 발송은 Phase 2)
- [ ] 딥링크로 해당 화면 이동 *(Phase 2)*

> **현재 상태**: 알림 설정 UI 및 DB 저장은 구현됨. 실제 푸시 알림 발송은 미구현.

---

#### 4.3.14 동네 기반 추천 *(부분 구현)*

**위치 설정 (현재 구현):**
- ~~GPS 자동 감지~~ → 카카오맵 검색으로 수동 설정
- 1개 지역만 등록 (시/도 + 시/군/구 + 좌표)

**추천 알고리즘 (현재 구현):**
- 프로필 좌표 기반 Haversine 거리 계산
- 반경 내 모집중 스터디 표시
- Fallback: 근처 스터디 없으면 전체 모집중 스터디 표시
