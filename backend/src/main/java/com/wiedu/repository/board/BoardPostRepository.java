...
public interface BoardPostRepository extends JpaRepository<BoardPost, Long> {
    ...
    /**
     * 댓글 수 atomic 감소 (Lost Update 방지, 0 미만 방지)
     */
    @Modifying
    @Query("UPDATE BoardPost p SET p.commentCount = CASE WHEN p.commentCount > 0 THEN p.commentCount - 1 ELSE 0 END WHERE p.id = :id")
